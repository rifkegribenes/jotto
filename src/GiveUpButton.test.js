...
/**
 * Factory function to create a ShallowWrapper for the GiveUpButton component
 * @function setup
...
test('calls `giveUp` prop upon button click', () => {
  // create a mock function so we can see whether it's called on click
  const giveUpMock = jest.fn();
  const wrapper = setup({ display: true, giveUp: giveUpMock });
